return {
	...
	config = function()
		...
		-- java_cmd = "java"
		-- java_cmd = os.getenv("JAVA_HOME") and os.getenv("JAVA_HOME") .. "/bin/java" or "java"
		java_cmd = "/Library/Java/JavaVirtualMachines/jdk-24.jdk/Contents/Home/bin"
		-- if not os.getenv("JAVA_HOME") then
		-- 	vim.notify("Warning: JAVA_HOME not set, falling back to system Java", vim.log.levels.WARN)
		-- end
		...
		bundles = {
			vim.fn.glob(
				"/Users/mohammadrehan/.local/share/nvim/mason/packages/java-debug-adapter/extension/server/com.microsoft.java.debug.plugin-*.jar"
			),
		}
		-- bundles = {
		-- 	vim.fn.glob(
		...
		vim.list_extend(
			bundles,
			vim.split(
				vim.fn.glob("/Users/mohammadrehan/.local/share/nvim/mason/packages/java-test/extension/server/*.jar"),
				-- vim.fn.glob(mason_registry.get_package("java-test"):get_install_path() .. "/extension/server/*.jar"),
				"\n"
			)
		)

		jdtls_path = "/Users/mohammadrehan/.local/share/nvim/mason/packages/jdtls"
